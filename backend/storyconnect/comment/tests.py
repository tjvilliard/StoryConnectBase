...
class CommentManagerTestCase(TestCase):
    def setUp(self):
        ...
        self.selection2 = TextSelection.objects.create(
            chapter=self.chapter, offset=10, offset_end=20, text='Test text', floating=True
        )
        ...
        self.comment_dismissed = WriterFeedback.objects.create(
            user=self.user, selection=self.selection2, comment='Test comment 2', dismissed=True
        )
...
class WriterFeedbackSerializerTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create(username='testuser')
        self.chapter = Chapter.objects.create(book=Book.objects.create(title="Test Book", owner=self.user))
        self.selection_data = {
            'chapter': self.chapter.id,
            'offset': 10,
            'offset_end': 20,
