...
class LibraryPage(APIView):
    def get(self, request, user_id):
        user_books = book_models.Book.objects.filter(user=user_id)
        # context = {}
...
class WriterFeedbackPage(APIView):
    def get(self, request, user_id, book_id):
        writer_books = book_models.Book.objects.filter(
            user=user_id
        )  # for the drop down
        ...
        writer_books_serializer = book_serializers.BookSerializer(
            writer_books, many=True
        )
        book_feedback_serializer = book_serializers.BookSerializer(
            book_feedback, many=False
        )
        chapter_feedback_serializer = book_serializers.ChapterSerializer(
            chapter_feedback, many=True
        )
...
class BookDetailPage(APIView):
    def get(self, request, book_id):
        ...
        book_details_serializer = book_serializers.BookSerializer(
            book_details, many=False
        )
        characters_serializer = book_serializers.CharacterSerializer(
...
class DemographicsPage(APIView):
    def get(self, request, user_id, book_id):
        ...
        unique_readers_serializer = book_serializers.BookSerializer(
            unique_readers, many=False
        )
        chapter_comments_serializer = comment_serializers.WriterFeedbackSerializer(
            chapter_comments, many=True
        )
        content = {
