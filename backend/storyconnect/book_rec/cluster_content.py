...
def recommend(df,id, title):
    ...
    for index, row in rec.iterrows():
        rec_entry = {'id':title_idx, 'title': title,'recommendation' :row['title'], 'rec_id':row['id']}
...
def recommend_all_books(df):
     for index, row in df.iterrows():
        ...
        for rec in get_book_recs:
            book_rec, created = bookrec_models.Book_Based_Rec.objects.get_or_create(book=the_book, recommendations=rec)
