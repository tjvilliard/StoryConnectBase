...
def user_logged_out_callback(sender, user, request):
    ...
    for book in all_user_books:
        book_genre, created = features_models.GenreTagging.objects.get_or_create(
            book=book
        )
        ...
        for chap in book.get_chapters():
            (
                chapter_genre,
                created,
            ) = features_models.ChapterTagging.objects.get_or_create(chapter=chap.pk)
            updated_chapter_len = len(chap.content)
            if (
                updated_chapter_len >= 2 * chapter_genre.last_chapter_len
                and updated_chapter_len >= 1000
            ):
                chapter_df = ct.predict_genre(
                    book_id=book, chapter_num=chap.chapter_number
                )
